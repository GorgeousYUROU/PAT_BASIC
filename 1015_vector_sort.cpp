...
for(int i=0;i<n;i++)
{
    ...
    stu.de=De;
    stu.cai=Cai;
    if((De>=H)&&(Cai>=H)) {v1.push_back(stu);continue;}
    if((De>=H)&&(Cai>=L)) {v2.push_back(stu);continue;}
    if((De>=L)&&(Cai>=L)&&(De>=Cai)) {v3.push_back(stu);continue;}
    if((De>=L)&&(Cai>=L)) {v4.push_back(stu);continue;}
}
int goods=v1.size()+v2.size()+v3.size()+v4.size();
cout<<goods<<endl;
